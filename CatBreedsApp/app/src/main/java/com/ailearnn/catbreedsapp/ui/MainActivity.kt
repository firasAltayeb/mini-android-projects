...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            //}
            val breedsData by viewModel.breedsLiveData.collectAsState()
            val breedsList = breedsData.data ?: emptyList()
                ...
                Surface(
                    ...
                    NavHost(
                        ...
                        composable(
                            ...
                            HomeScreen(breedsList = breedsList) { breed ->
                                navController.navigate("$DETAILS_SCREEN/${breed.id}")
                            }
                        }
                        composable(route = "$DETAILS_SCREEN/{breedId}") {
                            val breedId = it.arguments?.getString("breedId") ?: breedsList[0].id
                            DetailsScreen(breedsList = breedsList, breedId)
                        }
                    }
                }
            }
